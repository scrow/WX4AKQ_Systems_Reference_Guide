...
\date              {01-Sep-2014}
\newcommand\docver {Version 2014.7}
...
\begin{verbatim}
 ...
 ~/git/www-resources  # Resource submodule
\end{verbatim}
...
If there is not already a test workspace for the web site to be modified, create one.
...
Initialize submodules and check out the proper commits:
...
Link the \texttt{globals.php} file:
...
Changes should never be committed directly to the \texttt{master} branch!  Always create a new remote branch for the set of changes you are about to make and be sure to push that branch to the server.

First, check out the \texttt{master} branch and pull from the server:
...
Run \lstinline!git commit! if there are uncommitted changes.
...
Finally, update submodules and check out the proper commit for each:
...
...along with a corresponding modification to the submodules:
